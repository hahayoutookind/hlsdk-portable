...
static int NumBotModels = sizeof(BotModels) / sizeof(BotModels[0]);
...
void CBot::Spawn()
{
	...
	// hahayoutookind: Added custom color and model selection depending on the bot name
	SetBotColors(STRING(pev->netname));
	SetBotModelFromName(STRING(pev->netname));

	SET_CLIENT_KEY_VALUE(ENTINDEX(edict()), GET_INFO_BUFFER(edict()), "topcolor", "50");
	SET_CLIENT_KEY_VALUE(ENTINDEX(edict()), GET_INFO_BUFFER(edict()), "bottomcolor", "75");

	// Allow derived classes to setup at spawn time
	SpawnBot();
}

void CBot::SetBotModelFromName(const char *name)
{
	unsigned int hash = HashName(name);

	int index = hash % NumBotModels;

	char finalmodel[64];

	sprintf(finalmodel, "%s", BotModels[index]);

	SET_CLIENT_KEY_VALUE(ENTINDEX(edict()), GET_INFO_BUFFER(edict()), "model", finalmodel);
}

unsigned int CBot::HashName(const char *name)
{
...
void CBot::SetBotColors(const char *name)
{
	unsigned int hash = HashName(name);
	...
	//hahayoutookind: the commented code is broken
// 	sprintf(topString, "%d", top);
//	sprintf(topString, "%d", bottom);

	snprintf(topString, sizeof(topString), "%d", top);
	snprintf(bottomString, sizeof(bottomString), "%d", bottom);

	SET_CLIENT_KEY_VALUE(ENTINDEX(edict()), GET_INFO_BUFFER(edict()), "topcolor", topString);
	SET_CLIENT_KEY_VALUE(ENTINDEX(edict()), GET_INFO_BUFFER(edict()), "bottomcolor", bottomString);
}
...
bool CBot::IsEnemy(CBaseEntity *ent) const
{
	...
	CBasePlayer *self = static_cast<CBasePlayer *>(const_cast<CBot *>(this));
	CBasePlayer *other = static_cast<CBasePlayer *>(ent);
	if (g_pGameRules && g_pGameRules->PlayerRelationship(self, other) == GR_TEAMMATE)
		return false;
